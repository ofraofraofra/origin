#include "Is_triangle1.h"
	...
	Is_triangle::Is_triangle(int a, int b, int с, int A, int B, int С) : Triangle(a, b, с, A, B, С) {
		if (A != C) throw WrongFigure("Ошибка создания фигуры. Причина: сторона A не равна стороне C");
		if (a != c) throw WrongFigure("Ошибка создания фигуры. Причина: угол а не равен углу с");
	}
